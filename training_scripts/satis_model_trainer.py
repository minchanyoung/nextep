...
def create_advanced_features_satis(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 경력/단계
    df["career_length"] = (df["age"] - df.groupby("pid")["age"].transform("min")).clip(
        lower=0
    )
    df["career_stage"] = pd.cut(
        df["age"], bins=[0, 29, 39, 49, 100], labels=[1, 2, 3, 4], right=True
    ).astype(int)
...
def split_and_add_stats(df: pd.DataFrame):
    ...
    print(
        f"Train set: {len(train_df)} ({int(train_df['year'].min())}-{int(train_df['year'].max())})"
    )
    print(
        f"Validation set: {len(val_df)} ({int(val_df['year'].min())}-{int(val_df['year'].max())})"
    )
...
def main():
    ...
    gen_features = [
        "satisfaction_lag_1",
        "income_lag_1",
        "satisfaction_roll_mean_3",
        "satisfaction_roll_std_3",
        "income_roll_mean_3",
        "career_length",
        "career_stage",
        "satisfaction_mean",
    ...
    cat_cols = [
        c
        for c in ["gender", "education", "job_category", "career_stage"]
    ...
    models["xgb"], val_r2["xgb"] = train_one(
        xgb_model, X_train, y_train, X_val, y_val, "XGBoost"
    )
    models["lgb"], val_r2["lgb"] = train_one(
        lgb_model, X_train, y_train, X_val, y_val, "LightGBM"
    )
    models["cat"], val_r2["cat"] = train_one(
        cat_model, X_train, y_train, X_val, y_val, "CatBoost", cat_features_idx=cat_idx
    )
    ...
    models["lgb"].booster_.save_model(
        os.path.join(save_dir, "final_lgb_satis_model.txt")
    )
